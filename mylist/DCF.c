...
int main()
{
	clock_t start,finish,end;
   	clock_t total;
	struct DynamicCountFilter mydcf;
	unsigned CBFV_Counter_Size = 8;
	unsigned length = 64;
	char hashNum = 3;
	double Lambda = 1;
	if(!DCFInit(&mydcf, CBFV_Counter_Size, length, hashNum, Lambda))
		{ printf("error\n"); }
	else
	{ printf("DCFInit!\n");  }
	
	const unsigned char* s = "number1";
	const unsigned char* s2 = "number2";
	const unsigned char* s3 = "number3";
	const unsigned char* s4 = "number4";
	...
	start = clock();
	for(i = 0; i<9999; i++){
		DCFInsert(&mydcf, a[i], len);
	}
	//	if(DCFMembershipQuery(&mydcf, s, len))
	//num = DCFMultiplicityQuery(&mydcf, s, len);
	...
	for(i = 0; i<9999; i++){
               // DCFDelete(&mydcf, a[i], len);
		DCFMultiplicityQuery(&mydcf, a[i], len);
        }

	//printf("times = %d", DCFMultiplicityQuery(&mydcf, a[20], len));
	end = clock();
